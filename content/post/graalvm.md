...
حالا فرض کنید به همین حالت که می‌توانید یک برنامه‌ی وب را وب‌فریم‌ورم‌ها بسازید، فریم‌ورکی هم وجود داشت که بتوانید به کمک آن یک زبان‌برنامه نویسی بسازید!
...
GraalVM علاوه بر کامپایلر JIT (که با نام Graal شناخته می‌شود)، دارای یک کامپایلر AOT نیز می‌باشد! نام این کامپایلر SubstrateVM است. اگر زبان شما توسط Truffle ساخته شود و بتواند قابل کامپایل با Graal باشد،  SubstrateVM می‌تواند از روی آن یک ایمیج Native تولید کند.

یعنی جاوا یا زبان های دیگری که روی Graal فعال خواهند بود، علاوه بر اینکه می‌توانند روی JVM اجرا شوند، خواهند توانست به طور Native و بدون نیاز به VM نیز اجرا شوند. البته وقتی می‌گوییم به طور Native اجرا می‌شوند، انتظار چیزی در حد و اندازه ++C/C یا Go یا Swift یا Rust را نداشته باشید. این AOT چیزی شبیه ART در اندروید است:

- سیستم runtime و gc در ایمیج وجود دارند و جایی نرفته اند.
- استارتاپ برنامه‌ها با AOT، چندین برابر (حتی گاهی بیشتر از ده برابر) سریعتر از استارتاپ JVM خواهد بود.
- مصرف حافظه با AOT، بسیار پایین‌تر از JVM خواهد بود.
- به طور پیش‌فرض دارای JIT نخواهید شد، بنابراین ممکن است سرعتی پایین‌تر از JVM داشته باشید. (فقط ممکن است، در تمام شرایط اینطور نیست).
...
فرضا می‌توانید برنامه‌های سررور را با JVM اجرا کنید ولی برای برنامه‌های ترمینال یا GUI ، یک ایمیج Native تهیه کنید تا سبک‌تر باشند.
...
- GraalVM دارای یک کامپایلر جدید JIT با نام Graal است که می‌تواند جایگزین C2 شود.
- GraalVM دارای یک کامپایلر AOT با نام SubstrateVM است که می‌تواند برای زبان‌های ساخته شده با Truffle ایمیج‌های Native تولید کند.
